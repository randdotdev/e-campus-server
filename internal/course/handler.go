...
func (h *Handler) CreateLesson(c *gin.Context) {
	...
	section, err := h.service.GetSection(c.Request.Context(), req.SectionID)
	if errors.Is(err, ErrSectionNotFound) {
		response.NotFound(c, "section not found")
		return
	} else if err != nil {
	...
	lesson, err := h.service.CreateLesson(c.Request.Context(), req)
	if errors.Is(err, ErrDuplicateLesson) {
		response.Conflict(c, "lesson with this order index already exists in the section")
		return
	} else if err != nil {
	...
	response.Created(c, ToLessonResponse(lesson, time.Now()))
}
...
func (h *Handler) ListLessonsBySection(c *gin.Context) {
	...
	if lessonType := c.Query("type"); lessonType != "" {
		filters.Type = &lessonType
	}

	lessons, err := h.service.ListLessons(c.Request.Context(), filters)
	if err != nil {
	...
	response.OK(c, ToLessonsResponse(lessons, time.Now()))
}

func (h *Handler) ListLessonsByOffering(c *gin.Context) {
	offeringID, err := uuid.Parse(c.Param("offering_id"))
	if err != nil {
		response.BadRequest(c, "invalid offering id")
		return
...
func (h *Handler) UpdateLesson(c *gin.Context) {
	...
	userID := middleware.GetUserID(c)
	teacher, err := h.service.GetTeacherRole(c.Request.Context(), lesson.OfferingID, userID)
	if errors.Is(err, ErrTeacherNotFound) {
		response.Forbidden(c, "not a teacher of this course")
		return
	} else if err != nil {
	...
	updated, err := h.service.UpdateLesson(c.Request.Context(), lessonID, req)
	if err != nil {
	...
	response.OK(c, ToLessonResponse(updated, time.Now()))
}
...
func (h *Handler) DeleteLesson(c *gin.Context) {
	...
	err = h.service.DeleteLesson(c.Request.Context(), lessonID)
	if errors.Is(err, ErrLessonNotFound) {
		response.NotFound(c, "lesson not found")
	} else if err != nil {
