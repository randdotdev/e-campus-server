// Package http is the HTTP transport for the communication domain.
package http
...
type MuteInOfferingRequest struct {
...
type MuteResponse struct {
	...
	ScopeType   communication.MuteScope `json:"scope_type"`
...
func toMuteResponse(m *communication.Mute, now time.Time) MuteResponse {
	return MuteResponse{
		ID:        m.ID,
		UserID:    m.UserID,
		...
		IsActive:  m.Active(now),
	}
}

func toMuteWithUserResponse(m *communication.MuteWithUser, now time.Time) MuteResponse {
	resp := toMuteResponse(&m.Mute, now)
	resp.User = &UserBrief{ID: m.UserID, Name: m.UserName, NameLocal: m.UserNameLocal, Email: m.UserEmail}
	resp.MutedByUser = &UserBrief{ID: m.MutedBy, Name: m.MutedByName}
	if m.ScopeID != nil && m.OfferingName != nil {
		resp.Offering = &OfferingBrief{ID: *m.ScopeID, Name: *m.OfferingName}
	}
	return resp
}

func toMuteWithUserResponses(mutes []communication.MuteWithUser, now time.Time) []MuteResponse {
